...
class ChainedHashTable(Set):
    ...
    def remove(self, key: int) -> object:
        bin = self._hash(key)
        for j in range(self.t[bin].size()):
            if self.t[bin].get(j).key == key:
                remove_val = self.t[bin].remove(j)
                self.n -= 1
    ...
    def resize(self):
        ...
        for j in range(len(self.t)):
            for k in range(self.t[j].size()):
                bin = self._hash(self.t[j].get(k).key)
                temp[bin].add(0, self.t[j].get(k))
        self.t = temp
